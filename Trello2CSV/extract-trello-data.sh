...
cat ${INPUT} | jq -r '
    (
        ["Short ID", "Item", "Background", "Short URL", "List", "Card Closed", "List Closed", "Labels"], 
        ...
        .cards[] | 
        [.idShort, .name, .desc, .shortUrl, $listNames[.idList], .closed, $listStatuses[.idList], $listStatuses[.labels[].id]]
    ) | @csv' > ${OUTPUT}